...
async function getUserById (user_id) {
    const [rows] = await db.query("SELECT user_id, username, password, role FROM users WHERE user_id = ?", [user_id]);
    return rows[0];
}
...
async function deleteUserById (user_id) {
    try {
        await db.query('DELETE FROM users WHERE user_id = ? AND role = "user"', [user_id]);
    } catch (error) {
        console.error(error);
    }
}
